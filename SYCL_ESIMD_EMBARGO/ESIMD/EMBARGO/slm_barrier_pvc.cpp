//==-------------- slm_barrier_pvc.cpp  - DPC++ ESIMD on-device test -------==//
//
...
void load_to_slm(uint grpSize, uint localId, uint slmOffset, char *addr,
  ...
  uint numTotalBlocks = esimd_emu_impl_udiv(size, 256);
  uint numBlocks = esimd_emu_impl_udiv(numTotalBlocks, grpSize);
  uint numLeftOver = esimd_emu_impl_urem(numTotalBlocks, grpSize);
...
int main(void) {
  ...
  cl::sycl::range<1> GroupRange{GLOBAL_SIZE};

  // We need that many threads in each workgroup
  cl::sycl::range<1> TaskRange{LOCAL_SIZE};
  cl::sycl::nd_range<1> Range{GroupRange * TaskRange, TaskRange};
  ...
  auto e = q.submit([&](handler &cgh) {
    cgh.parallel_for<class Test>(
        Range, [=](cl::sycl::nd_item<1> ndi) SYCL_ESIMD_KERNEL {
          ...
          auto shiftID = esimd_emu_impl_urem((localID + 1), 4);
  ...
  std::cout << (result < 0 ? "Slm_barrier_v2 Test FAILED\n"
                           : "Slm_barrier_v2 Test Passed\n");
  return result < 0 ? 1 : 0;
}
